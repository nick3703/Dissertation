...
as given in \eqref{eq:SpatCov}.
	...
	\begin{figure}[!htp]
		...
		\isucaption[Comparison of Laplace approximation and extended Laplace approximation]{Recommended methodology for inference for subspace of parameters}\label{fig:Fitting}
	\end{figure}
	 ...
	 \begin{figure}[!htp]
	 	\centering
	 	\includegraphics[width=0.8\linewidth, height=0.5\textheight]{Fitting2}
	 	\isucaption[Marginal credible intervals using extended Laplace Approximation]{95\% credible interivals of $\sigma^2_{sp}$ found for combinations of $\sigma^2_{sp}$ and $\eta$.  Here the fourth order extended Laplace approximation was used.  The colors of the parameter values correspond to the maximum $g^{ii}$ terms at the posterior mode.  From here we can see that $g^{ii}>2$ should cause concern in the inference.}\label{fig:Fitting2}
	 \end{figure}
	...
	\begin{table}[h]
		\begin{center}
			...
			\end{tabular}
		\end{center}
		\isucaption[Comparison of credible intervals between extended Laplace and MCMC]{Comparison between 95 \% credible intervals formed using Extended LA and MCMC.  Note that the 95 \% credible intervals for Expanded LA were donethrough using finite differences to approximate the Hessian and then using a Gaussian approximation to the posterior.}\label{Table:Results2}
	\end{table}
