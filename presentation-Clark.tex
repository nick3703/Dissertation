...
\begin{frame}
	\frametitle{A Model of Criminal Behavior (Short et al. 2008)}
		\begin{itemize}
			...
			\item $s_i \in \{s_1,\cdots,s_n\}$ - fixed regions in $\mathbb{R}^2$
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Commonly discretized as
			...
			\end{align}
		\item $\eta$ essentially determines $p$ in INGARCH$(0,p)$, $\eta<.5$ for stationarity
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Short (2008) Extension - Spatial Spread}
		...
		\begin{itemize}
			...
			\item  Recall $A(s_i,t)\equiv A(s_i,0)+B(s_i,t)$ as attractiveness, let $N_{s_i}$ be spatial adjacency matrix
			\begin{align}
			B(s_i,t)& =\kappa B(s_i,t- \Delta t) + \frac{\psi}{|N_{s_i}|}\sum_{s_j \in N_{s_i}} \left[B(s_j,t- \Delta t)-B(s_i,t- \Delta t)\right]\nonumber\\
			& +\eta Z(s_i,t- \Delta t)
			\end{align}
...
\end{frame}		
   ...
   \begin{frame}
   	...
   	& \boldsymbol{U} \sim \mbox{Gau} (\boldsymbol{0},[\sigma_{sp}^2(\boldsymbol{N}-\boldsymbol{C})]^{-1})\nonumber
   	\end{align}
   	\begin{itemize}
   		\item Removed temporal trend and seasonality
   		\item Single spatial effect, $\zeta$ fixed near edge of parameter space
    ...
    \begin{table}[h]
    	\begin{center}
    		\begin{tabular}{ |c|c|c| } 
    			...
    			$\beta_{ym}$ & \highlight{(-0.77,0.16)}& \highlight{(0.06,0.09)}\\
    			$\beta_{wealth}$&\highlight{(0.04,0.16)} & \highlight{(-0.04,0.01)}\\
    			$\beta_{unemp}$ & (0.005,0.07)& (0.002,0.03) \\
    			$\eta$ & \highlight{(0.02,0.05)} & \highlight{(0.22,0.24)}\\
    			$\kappa$ & (0.42,0.49)& (0.44,0.48)\\
    			$\sigma_{sp}^2$ & (0.36,0.50) & - \\
    			$\sigma_{ind}^2$& (0.49,0.57)& - \\
    			\hline
    		\end{tabular}
    	\end{center}
    \end{table}
 ...
 \label{Table:Pvals} 
  		\begin{center}
  			\begin{tabular}{ |l|c|c| } 
  				...
  				$p_2$ - Variance to Mean Ratio & 0.42 & 0\\
  	...
  	\end{table}
  	\begin{itemize}
  		\item INGARCH(1,1)/Short Model unable to replicate second order properties
  		\item Correct Lag 1 Auto Cor dictates incorrect Var/Mean ratio
  		\item Conclusions on self-excitement
  	\end{itemize}
  \end{frame}
