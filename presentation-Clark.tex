...
\begin{frame}
	\frametitle{Short (2008) Extension - Spatial Spread}
		...
		\begin{itemize}
			...
			& +\eta Z(s_i,t- \Delta t)
			\end{align}
			\item Four factors impact change in crime rate, base attractiveness, decay  $\kappa$, and repeat victimization, $\eta$, and spatial spread $\psi$
...
\end{frame}
    ...
    \begin{frame}
    	...
    	Implies Lag-one Cor in (.5,.707)
    	\newline
    	
    	For fixed Var-Mean Ratio at 1.2, Implies Lag-one Cor in (.2,.29)
    	\newline
    	
    	Actual crime data, Var-Mean is 2, Lag-one Cor is .3
    \end{frame}
...
\pause
  ...
  \begin{frame}
  	\frametitle{Relationship to INGARCH(1,1)/Short Model}
	...
	\item Assume each time period, exogeneous impact is stochastic and spatially correlated yields SPINGARCH
  		...
  		\end{equation}
  		\item Change in intensity due to three factors, spatially correlated exogeneous, natural decay, and excitement
  		\end{itemize}
...
\end{figure}
 ...
 \end{align}
  ...
  \begin{frame}
  	\frametitle{Model Properties - Parameter Space}
  	\begin{itemize}
  		\item $Z(s_i,t)|Y(s_i,t),\mathcal{H}_{Z(s_i)}\sim\mbox{Pois }(\lambda(s_i,t))$ where $\mathcal{H}_{Z(s_i)}$ is history of process at location $s_i$
  		\begin{align}
  			\lambda(s_i,t)&=\exp \left[ Y(s_i,t) \right] + \eta Z(s_i,t-1) + \kappa \lambda(s_i,t-1)\nonumber\\
  ...
  \end{frame}
    ...
    \begin{frame}
    	\frametitle{Model as Markov Chain}
    	\begin{itemize}
   ...
   \end{frame}
    ...
    \begin{frame}
    	...
    	\begin{itemize}
    	  ...
    	  \item Extended LA/LA - 10 minutes 
    	  \end{itemize}
    \end{frame}
